...
int save16BitTiff(unsigned short* imageData, int nRows, int nColumns, const char* fileName) {
    ...
    NSBitmapImageRep *bitmap = [[NSBitmapImageRep alloc]
        ...
        bytesPerRow:nColumns*2
        bitsPerPixel:16];
    
    // Copy the image data to the bitmap representation
    memcpy([bitmap bitmapData], imageData, nRows*nColumns*2);
    
    // Create a dictionary of TIFF export options
    NSDictionary *tiffExportOptions = @{
    ...
    NSError *error;
    if (![tiffData writeToFile:[NSString stringWithUTF8String:fileName]
                       options:NSDataWritingAtomic
                         error:&error]) {
        //NSLog(@"Failed to save TIFF file: %@", error);
        return FILE_ERR;
    }
    return NO_ERR;
}
...
int save16BitColorTIFF(unsigned short* rgbData, int nRows, int nColumns, const char *fileName) {
    ...
    // Create an NSBitmapImageRep with the image data
...
/*
SAVETIFF filename
 Saves a 16-bit compressed TIF image based on the current data and cmin/cmax values. Does not alter the current image.
*/

int savetiff_c(int n, char* args)
{
#ifdef MacOSX_UI
    ...
    Image im;
    im << iBuffer;
    snprintf(theArg,128,"%f %f %s", UIData.cmin,UIData.cmax,args);
    printMax=0;
    satiffscaled_c(0,theArg); // remap iBuffer to fill 16 bits based on cmin and cmax
    printMax=1;
    int err = iBuffer.err();
    iBuffer.free();
    iBuffer = im;   // return original image
    iBuffer.getmaxx(0);
    update_UI();
    return err;
...
int satiff_c(int n, char* args)
{
    ...
    int nRows = iBuffer.rows();
    int nCols = iBuffer.cols();

    // Define the file name
    const char *fileName = fullname(args,TIF_DATA);
    printf("Writing TIFF to file: %s\n",fileName);
    map_c(0,(char*)"0 65535"); // remap to fill 16 bits
    DATAWORD* theData=iBuffer.getImageData();
    if(iBuffer.isColor()){
        nRows /= 3;
        unsigned short* imageData = new unsigned short[nRows*nCols*3];
        for (int i = 0; i < nRows * nCols * 3; i+= 3) {
            imageData[i] = *theData;
            imageData[i+1] = *(theData + nRows * nCols);
            imageData[i+2] = *(theData++ + nRows * nCols * 2);
        }
        int err = save16BitColorTIFF( imageData,  nRows,  nCols,  fileName);
        delete[] imageData;
        if(err){
            beep();
            printf("File Error Saving Tif Image.\n");
        }
        return err;

    }else {
        unsigned short* imageData = new unsigned short[nRows*nCols];
        for (int i = 0; i < nRows * nCols; i++) {
            imageData[i] = *theData++;
        }
        int err = save16BitTiff( imageData,  nRows,  nCols,  fileName);
        delete[] imageData;
        if(err){
            beep();
            printf("File Error Saving Tif Image.\n");
...
int satiffscaled_c(int n, char* args)
{
    ...
    printf("Writing TIFF to file: %s\n",txt);
    // modify the data according to min/max
    DATAWORD fpval,range = mymax - mymin;
    ...
    if(iBuffer.isColor()){
        ...
        int err = save16BitColorTIFF( imageData,  nRows,  nCols,  txt);
        delete[] imageData;
        if(err){
            beep();
            printf("File Error Saving Tif Image.\n");
        ...
        int err = save16BitTiff( imageData,  nRows,  nCols,  txt);
        delete[] imageData;
        if(err){
            beep();
            printf("File Error Saving Tif Image.\n");
